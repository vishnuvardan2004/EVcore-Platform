...
export const EmployeeManagement: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.employeeId?.trim()) {
      ...
      const duplicateEmpId = employees.find(emp => 
        emp.employeeId?.toUpperCase() === formData.employeeId.trim().toUpperCase() && 
    ...
    if (!formData.emailId?.trim()) {
      ...
      const duplicateEmail = employees.find(emp => 
        emp.emailId?.toLowerCase() === formData.emailId.trim().toLowerCase() && 
        emp.id !== editingEmployee?.id
      );
      if (duplicateEmail) {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const response = await dbApi.listEmployees();
      console.log('API Response:', response);
      
      // Backend returns { success: true, data: { documents, pagination } }
      if (response.success && response.data) {
        // The response.data should contain { documents: Employee[], pagination: {...} }
        const data = response.data as any;
        const employees = data.documents || [];
        
        // Always ensure we have at least one employee in dev mode for testing
        if (employees.length === 0 && config.IS_DEVELOPMENT) {
          ...
          const sanitizedData = employees.map((emp: any, index: number) => {
            ...
            emailId: emp.email || emp.emailId || 'N/A',  // Backend uses 'email', frontend uses 'emailId'
            ...
            designation: emp.position || emp.designation || 'N/A',  // Backend uses 'position', frontend uses 'designation'
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingEmployee) {
        ...
        // Map frontend field names to backend expected field names for updates
        const backendUpdateData = {
          ...formData,
          email: formData.emailId,  // Backend expects 'email' but frontend uses 'emailId'
          position: formData.designation  // Backend expects 'position' but frontend uses 'designation'
        };
        
        console.log('Mapped update data for backend:', JSON.stringify(backendUpdateData, null, 2));
        
        const response = await dbApi.updateEmployee(editingEmployee.id, backendUpdateData);
        ...
        console.log('Creating employee with form data:', JSON.stringify(formData, null, 2));
        console.log('Form data type validation:', {
          employeeId: typeof formData.employeeId,
          fullName: typeof formData.fullName,
          emailId: typeof formData.emailId,
        ...
        console.log('Mapped data for backend:', JSON.stringify(backendFormData, null, 2));
        
        const response = await dbApi.createEmployee(backendFormData as Omit<Employee, '_id' | 'createdAt' | 'updatedAt'>);
        console.log('Employee creation response:', response);
        
        if (response.success && response.data) {
          // The actual document is nested in response.data.document
          const createdEmployee = (response.data as any).document || response.data;
          
          // Defensive check to ensure the created employee has an ID
          if (!createdEmployee._id && !createdEmployee.id) {
            console.warn('Warning: Created employee does not have ID:', response.data);
            console.warn('Full response structure:', response);
            throw new Error(`Server did not return a valid employee ID. Response structure: ${JSON.stringify(Object.keys(response.data))}`);
          }
      ...
      // Extract meaningful error message with more detail
      ...
      if (error instanceof Error) {
        ...
        if ((error as any).message) {
          ...
          if (message.includes('E11000') && message.includes('email_1 dup key')) {
            errorMessage = "This email address is already registered. Please use a different email.";
          } else if (message.includes('E11000') && message.includes('employeeId_1 dup key')) {
            errorMessage = "This Employee ID is already in use. Please use a different Employee ID.";
          } else if (message.includes('E11000')) {
            errorMessage = "This record contains duplicate information. Please check your input.";
          } else {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const response = await dbApi.deleteEmployee(id);
      console.log('Employee deletion response:', response);
      ...
      if (response.success) {
        ...
        throw new Error('Failed to delete employee - invalid response');
      }
    } catch (error) {
