...
// Lazy load the main VehicleTracker component
const LazyVehicleTracker = React.lazy(() => import('./VehicleTracker'));

const LoadingFallback = () => (
  <div className="min-h-screen bg-gray-50 flex items-center justify-center">
    <div className="text-center">
      <Loader2 className="w-8 h-8 animate-spin mx-auto mb-4 text-blue-600" />
      <h2 className="text-lg font-semibold text-gray-900 mb-2">Loading Vehicle Tracker...</h2>
      <p className="text-gray-600">Initializing services and components</p>
    </div>
  </div>
);
...
const ServiceHealthIndicator: React.FC<{ health: ServiceHealth; onRetry: () => void }> = ({ 
  ...
  const criticalServices = ['database', 'authentication', 'localStorage'];
        ...
        <Card className="border-orange-200 bg-orange-50">
          <CardHeader className="pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-orange-900">Service Health Check</CardTitle>
                <p className="text-orange-700 text-sm mt-1">
                  {hasCriticalErrors 
                    ? 'Critical services are not available. Vehicle Tracker cannot load properly.' 
    ...
    <Card className="max-w-md w-full border-red-200 bg-red-50">
      <CardHeader className="pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-red-900">Vehicle Tracker Error</CardTitle>
            <p className="text-red-700 text-sm mt-1">
              An unexpected error occurred while loading the Vehicle Deployment Tracker.
            </p>
          </div>
        </div>
      </CardHeader>
...
export const VehicleTrackerWrapper: React.FC = () => {
  ...
  const performHealthCheck = async () => {
    ...
    try {
      ...
      const criticalServices = ['database', 'localStorage'];
