...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      try {
        console.log('🔍 Token found, attempting to validate...');
        setUser(null);

        // Check if token is JWT format (3 parts)
        if (token.split('.').length === 3) {
          console.log('✅ JWT format detected, validating with API...');
          try {
            ...
            console.log('❌ API validation failed, clearing token');
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    console.log('❌ Login failed: Authentication rejected');
    return false;
  };
  ...
  const canAccessFeature = (featureId: string): boolean => {
    ...
    // New 6-module access control system
    const roleFeatureAccess: Record<string, string[]> = {
      super_admin: [
        // Full access to all 6 core modules
        'vehicle_deployment', 'smart_bookings', 'data_hub', 
        'driver_onboarding', 'trip_analytics', 'energy_management',
        // Administrative modules
        'audit_logs', 'admin_settings',
        // Legacy features
        'dashboard', 'database_management', 'global_reports'
      ],
      admin: [
        // All 6 core modules except audit logs
        'vehicle_deployment', 'smart_bookings', 'data_hub',
        'driver_onboarding', 'trip_analytics', 'energy_management',
      ...
      employee: [
        // 5 out of 6 core modules (no data_hub)
        'vehicle_deployment', 'smart_bookings', 
        'driver_onboarding', 'trip_analytics', 'energy_management',
      ...
      pilot: [
        // Only 2 core modules
        'trip_analytics', 'energy_management',
